...
def path_generator(config_file, target_file, result_folder, if_animate=False, if_write_to_file=False):
    ...
    sim_length = 210            # Number of steps
    dt = 0.25                      # Time step duration
    max_speed_alpha = 2*math.pi*2.78/60  # Maximum RPM #0.2911
    min_speed_alpha = 2*math.pi*0.73/60  # Minimum RPM #0.0764
    max_speed_beta = 2*math.pi*3.75/60  # Maximum RPM #0.3927
    min_speed_beta = 2*math.pi*0.98/60  # Minimum RPM #0.1026
    ...
    if target_from_file:
        ...
        for i, line in enumerate(fr2):
            ...
            x1,x2,x3,x4,x5,x6,x7 = map(float, line.split())
